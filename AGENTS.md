...
curl -s localhost:4317/api/projects \
  ...
  -d '{"name":"Ops Guide","description":"Dispatch and inventory tooling"}'
```
...
```bash
curl -s localhost:4317/api/projects/ops-guide/items \
  -H 'content-type: application/json' \
  -d '[
...
```bash
curl -s localhost:4317/api/projects/ops-guide
```
...
curl -s -X PATCH localhost:4317/api/items/<id>/checks/step-7 \
  -H 'content-type: application/json' \
  -d '{"result":"fail","note":"what happened","by":"billy"}'
```
...
curl -s -X PATCH localhost:4317/api/settings \
  -H 'content-type: application/json' \
  -d '{"onboardedAt":"2026-09-16T20:15:00Z","autoCapture":true,"checkInOnStart":true,"postFindings":true,"summariseOnExit":false,"defaultProject":"ops-guide"}'
```
