...
- [x] This changes the tool only: no project data.
...
describe('pull request description rule', () => {
  ...
  it('refuses an empty body', () => {
    expect(checkPrDescription('').problems).toHaveLength(3);
  });
  ...
  it('refuses a description whose tool-only box is not ticked', () => {
    const result = checkPrDescription(good.replace('- [x] This', '- [ ] This'));
    expect(result.ok).toBe(false);
    expect(result.problems[0]).toContain('tool only');
  });
